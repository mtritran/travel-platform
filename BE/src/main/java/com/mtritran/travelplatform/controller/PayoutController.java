...
public class PayoutController {
    ...
    public ApiResponse<PayoutRequest> createRequest(@Valid @RequestBody PayoutCreateRequest request) {
        return ApiResponse.<PayoutRequest>builder()
                .result(payoutService.createRequest(request.getAmount(), request.getBankName(), request.getBankAccountNumber(), request.getBankAccountName(), request.getPaymentPin()))
                .build();
    }
